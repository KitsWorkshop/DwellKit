...
- [ ] **Dry-run the full fan-out at real class size** (~30 min) — `dwellkit class` is verified at 2 students; 30 concurrent has not been run
  Rate limits look comfortable on paper (~450–650 REST calls for 30 students against 5,000/hour — roughly 13–21 per student, plus preflight and one existence check per roster row) but a 30-repo burst has never actually been run. Watch for secondary rate limits, which are triggered by burst *concurrency* rather than total volume and are not visible in the standard rate-limit headers.
...
- [x] ~~**Fix silent row loss on files without a trailing newline**~~ → `while read` returns non-zero at EOF without a line terminator, so the final line was read into the variables but never processed. A roster saved by an editor that omits the trailing newline silently dropped its **last student** — reporting `valid: 19` for 20 rows, with no error. Fixed in both the roster and results readers.
